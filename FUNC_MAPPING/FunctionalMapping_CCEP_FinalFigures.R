...
source('/Users/cce3182/Desktop/b1134/tools/eeganalysis/STIM/CCEP_LoadData.R')
source('/Users/cce3182/Desktop/b1134/tools/eeganalysis/STIM/CCEP_CaseList.R')
source('/Users/cce3182/Desktop/b1134/tools/eeganalysis/STIM/CCEP_SiteList.R')
source('/Users/cce3182/Desktop/b1134/tools/eeganalysis/STIM/CCEP_BuildTable.R')
...
source('/Users/cce3182/Desktop/b1134/tools/eeganalysis/FUNC_MAPPING/FunctionalMapping_BuildTable.R')
############################################## load functional mapping data
InfoTable <- FunctionalMapping_LoadData()
SiteSummary <- FunctionalMapping_Summary(InfoTable)
FM_Data <- FunctionalMapping_BuildTable(SiteSummary)

############################################## load CCEP data
AverageInfo_Table <- CCEP_LoadData()
structure <- CCEP_CaseList(AverageInfo_Table)
AverageInfo_Table <- structure$AverageInfo_Table
case_list <- structure$case_list
rm(structure)
site_list <- CCEP_SiteList(case_list)
CCEP_Data <- CCEP_BuildTable(site_list)

############################## Figure 2
Plot_info <- CCEP_Data[!CCEP_Data$TissueType %in% c('Subcortical') &
                         !(CCEP_Data$AmpResult %in% c('No In-Network Coverage')) &
                         !(CCEP_Data$AmpResult %in% c("Miss + Nonspecific",
...
stats <- list(t.test(Plot_info$DisttoWMBoundary[Plot_info$StimulationResult=='No Effects'], Plot_info$DisttoWMBoundary[Plot_info$StimulationResult=='Behavioral Effects'],alternative='greater'))
...
p1 <- ggplotGrob(ggplot(data = Plot_info, aes(x=as.numeric(AmpResult),y=DisttoWMBoundary, color = AmpResult)) +
   ...
   geom_boxplot(width=0.2, outlier.shape = NA, show.legend = FALSE) +
   gghalves::geom_half_point(side='l', alpha=0.5, size=1, transformation=position_jitter(width=0.05,height=0),show.legend = FALSE) +
   geom_bracket( xmin = c(1), xmax = c(2), y.position = c(4.5) ,label.size = 4, label = 'p < 0.001',
                 inherit.aes = FALSE, tip.length = c(0.02)) +
...
p2 <- ggplotGrob(ggplot(data = Plot_info, aes(x=as.numeric(AmpResult),y=NearNetworks, color = AmpResult)) +
   ...
   geom_bracket( xmin = c(2), xmax = c(3), y.position = c(6.5) ,label.size = 4, label = 'p = 0.043',
                 inherit.aes = FALSE, tip.length = c(0.02)) +
   scale_y_continuous(limits=c(0,7), breaks=c(0:6)) +
...
Plot_info <- CCEP_Data[!CCEP_Data$TissueType %in% c('Subcortical','Out of Brain') &
                         !CCEP_Data$AmpResult %in% c('Not Stimulated','No In-Network Coverage','No Effect'),]
sublist <- unique(Plot_info$SubjectID)
Data_Table <- array(data=0, dim = c(6,6))
colnames(Data_Table) <- c('DNA','DNB','LANG','FPNA','SAL','UNI')
rownames(Data_Table) <- c('DNA','DNB','LANG','FPNA','SAL','UNI')
...
Permutted_Data_Table <- array(data=0, dim = c(6,6,10000))
colnames(Permutted_Data_Table) <- c('DNA','DNB','LANG','FPNA','SAL','UNI')
rownames(Permutted_Data_Table) <- c('DNA','DNB','LANG','FPNA','SAL','UNI')
for (permut in c(1:dim(Permutted_Data_Table)[3])){
  Permutted_Data <- Plot_info
  Permutted_Data$Stimulated_networks <- sample(Permutted_Data$Stimulated_networks, dim(Permutted_Data)[1])
...
for (i in c(1:dim(Plot_info)[1])){
  Plot_info$pvalue[i] <- sum(Permutted_Data_Table[Plot_info$Targeted[i],Plot_info$Stimulated[i],] >=
                               Data_Table[Plot_info$Targeted[i],Plot_info$Stimulated[i]])/dim(Permutted_Data_Table)[3]
...
ggplot(Plot_info, aes(x=Stimulated, y=Targeted, fill = value)) + 
  ...
  scale_x_discrete(labels=c('DN-A','DN-B','LANG','FPN-A','CG-OP','SM')) +
  scale_y_discrete(labels=c('DN-A','DN-B','LANG','FPN-A','CG-OP','SM')) +
...
for (site in c(1:dim(FM_Sensorimotor_Data)[1])){
  ...
  } else if (any(c('Hand','Body','Visual','Auditory','Touch','Face') %in% unlist(str_split((FM_Sensorimotor_Data$EffectList[site]),','))) &
...
Plot_info <- FM_Sensorimotor_Data %>% group_by(DisttoUNI,SMEffect)  %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  ungroup() %>% complete(DisttoUNI,SMEffect, fill = list(n=0,freq=0))
p1 <- ggplotGrob(ggplot(data = Plot_info, aes(x=DisttoUNI,y=n,fill=SMEffect)) +
         geom_bar(stat='identity') +
         scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
         scale_fill_manual(values=c("#377EB8", "#4DAF4A")) +
         theme_bw() +
...
nullsensorimotormodel <- lm( DisttoWMBoundary~DisttoUNI, Plot_info[Plot_info$SMEffect == 'No Effects',])
summary(nullsensorimotormodel)
sensorimotormodel <- lm(DisttoWMBoundary~DisttoUNI, Plot_info[Plot_info$SMEffect == 'Sensorimotor Effects',])
summary(sensorimotormodel)
p2 <- ggplotGrob(ggplot(data = Plot_info, aes(x=DisttoUNI)) +
                   annotate('rect',xmin=-Inf,xmax=Inf,ymin=0,ymax=Inf, fill='gray',alpha=0.5) +
                   geom_hline(yintercept = 0, linewidth=0.25) +
                   geom_line(aes(y=DisttoWMBoundary),stat="smooth",method = "lm", formula = y ~x, color='black', linewidth = 1,alpha = 0.5) + 
...
FM_Sensorimotor_Data <- FM_Sensorimotor_Data[!is.na(FM_Sensorimotor_Data$SMEffect),]
Plot_info <- FM_Sensorimotor_Data
Plot_info <- Plot_info[Plot_info$DisttoUNI <= 5,]
...
Plot_info$SMStim <- NA
Plot_info$CGOPStim <- NA
Plot_info$FPNAStim <- NA
Plot_info$LANGStim <- NA
Plot_info$DNBStim <- NA
Plot_info$DNAStim <- NA
for (site in c(1:dim(Plot_info)[1])){
  if (Plot_info$Stimulated_networks[site] == 'None'){
    Plot_info$SMStim[site] <- 'No Activity'
  } else if (c('UNI') %in% unlist(Plot_info$Stimulated_networks[site])){
    Plot_info$SMStim[site] <- 'Sensorimotor Network Activation'
  } else {
    Plot_info$SMStim[site] <- 'Other Network Activation'
  }
  if (Plot_info$Stimulated_networks[site] == 'None'){
    Plot_info$CGOPStim[site] <- 'No Activity'
  } else if (c('SAL') %in% unlist(Plot_info$Stimulated_networks[site])){
    Plot_info$CGOPStim[site] <- 'Cingulo-Opercular Network Activation'
  } else {
    Plot_info$CGOPStim[site] <- 'Other Network Activation'
  }
  if (Plot_info$Stimulated_networks[site] == 'None'){
    Plot_info$FPNAStim[site] <- 'No Activity'
  } else if (c('FPNA') %in% unlist(Plot_info$Stimulated_networks[site])){
    Plot_info$FPNAStim[site] <- 'Frontoparietal Network A Activation'
  } else {
    Plot_info$FPNAStim[site] <- 'Other Network Activation'
  }
  if (Plot_info$Stimulated_networks[site] == 'None'){
    Plot_info$LANGStim[site] <- 'No Activity'
  } else if (c('LANG') %in% unlist(Plot_info$Stimulated_networks[site])){
    Plot_info$LANGStim[site] <- 'Language Network Activation'
  } else {
    Plot_info$LANGStim[site] <- 'Other Network Activation'
  }
  if (Plot_info$Stimulated_networks[site] == 'None'){
    Plot_info$DNBStim[site] <- 'No Activity'
  } else if (c('DNB') %in% unlist(Plot_info$Stimulated_networks[site])){
    Plot_info$DNBStim[site] <- 'Default Network B Activation'
  } else {
    Plot_info$DNBStim[site] <- 'Other Network Activation'
  }
  if (Plot_info$Stimulated_networks[site] == 'None'){
    Plot_info$DNAStim[site] <- 'No Activity'
  } else if (c('DNA') %in% unlist(Plot_info$Stimulated_networks[site])){
    Plot_info$DNAStim[site] <- 'Default Network A Activation'
  } else {
    Plot_info$DNAStim[site] <- 'Other Network Activation'
  }
}
Plot_info$SMStim <- factor(Plot_info$SMStim, levels=c('No Activity','Other Network Activation','Sensorimotor Network Activation'))
Plot_info$CGOPStim <- factor(Plot_info$CGOPStim, levels=c('No Activity','Other Network Activation','Cingulo-Opercular Network Activation'))
Plot_info$FPNAStim <- factor(Plot_info$FPNAStim, levels=c('No Activity','Other Network Activation','Frontoparietal Network A Activation'))
Plot_info$LANGStim <- factor(Plot_info$LANGStim, levels=c('No Activity','Other Network Activation','Language Network Activation'))
Plot_info$DNBStim <- factor(Plot_info$DNBStim, levels=c('No Activity','Other Network Activation','Default Network B Activation'))
Plot_info$DNAStim <- factor(Plot_info$DNAStim, levels=c('No Activity','Other Network Activation','Default Network A Activation'))

Plot_table <- Plot_info[!is.na(Plot_info$SMStim),] %>% group_by(DisttoUNI,SMStim)  %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  ungroup() %>% complete(DisttoUNI,SMStim, fill = list(n=0,freq=0))
p1 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoUNI,y=n,fill=SMStim)) +
       geom_bar(stat='identity') +
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
      scale_y_continuous(limits=c(0,28),breaks=c(0,5,10,15,20,25)) +
       ...
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             panel.grid.minor = element_blank(),
             text = element_text(size = 12),legend.position = c(0.80,0.69),
             legend.title = element_blank(),legend.margin=margin(-0.2,0.1,0,0, "cm"),
             strip.text = element_text(margin = margin (3,0,3,0)),panel.spacing = unit(0, "lines"),
             strip.background =element_rect(fill=rgb(159,176,193,maxColorValue = 255))))
Plot_table <- Plot_info[!is.na(Plot_info$CGOPStim),] %>% group_by(DisttoSAL,CGOPStim)  %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  ungroup() %>% complete(DisttoSAL,CGOPStim, fill = list(n=0,freq=0))
p2 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoSAL,y=n,fill=CGOPStim)) +
       geom_bar(stat='identity') +
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(0,28),breaks=c(0,5,10,15,20,25)) +
       scale_fill_manual(values=c("#377EB8","#F8766D", "#4DAF4A")) +
       theme_bw() +
       labs(x = 'Distance to Cingulo-Opercular Network (mm)',y = '# of Sites') + 
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             panel.grid.minor = element_blank(),
             text = element_text(size = 12),legend.position = c(0.80,0.69),
             legend.title = element_blank(),legend.margin=margin(-0.2,0.1,0,0, "cm"),
             strip.text = element_text(margin = margin (3,0,3,0)),panel.spacing = unit(0, "lines"),
             strip.background =element_rect(fill=rgb(159,176,193,maxColorValue = 255))))
Plot_table <- Plot_info[!is.na(Plot_info$FPNAStim),] %>% group_by(DisttoFPNA,FPNAStim)  %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  ungroup() %>% complete(DisttoFPNA,FPNAStim, fill = list(n=0,freq=0))
p3 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoFPNA,y=n,fill=FPNAStim)) +
       geom_bar(stat='identity') +
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(0,28),breaks=c(0,5,10,15,20,25)) +
       scale_fill_manual(values=c("#377EB8","#F8766D", "#4DAF4A")) +
       theme_bw() +
       labs(x = 'Distance to Frontoparietal Network A (mm)',y = '# of Sites') + 
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             panel.grid.minor = element_blank(),
             text = element_text(size = 12),legend.position = c(0.80,0.69),
             legend.title = element_blank(),legend.margin=margin(-0.2,0.1,0,0, "cm"),
             strip.text = element_text(margin = margin (3,0,3,0)),panel.spacing = unit(0, "lines"),
             strip.background =element_rect(fill=rgb(159,176,193,maxColorValue = 255))))
Plot_table <- Plot_info[!is.na(Plot_info$LANGStim),] %>% group_by(DisttoLANG,LANGStim)  %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  ungroup() %>% complete(DisttoLANG,LANGStim, fill = list(n=0,freq=0))
p4 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoLANG,y=n,fill=LANGStim)) +
       geom_bar(stat='identity') +
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(0,28),breaks=c(0,5,10,15,20,25)) +
       scale_fill_manual(values=c("#377EB8","#F8766D", "#4DAF4A")) +
       ...
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             ...
             strip.text = element_text(margin = margin (3,0,3,0)),panel.spacing = unit(0, "lines"),
             strip.background =element_rect(fill=rgb(159,176,193,maxColorValue = 255))))
Plot_table <- Plot_info[!is.na(Plot_info$DNBStim),] %>% group_by(DisttoDNB,DNBStim)  %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  ungroup() %>% complete(DisttoDNB,DNBStim, fill = list(n=0,freq=0))
p5 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoDNB,y=n,fill=DNBStim)) +
       geom_bar(stat='identity') +
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(0,28),breaks=c(0,5,10,15,20,25)) +
       scale_fill_manual(values=c("#377EB8","#F8766D", "#4DAF4A")) +
       theme_bw() +
       labs(x = 'Distance to Default Network B (mm)',y = '# of Sites') + 
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             panel.grid.minor = element_blank(),
             text = element_text(size = 12),legend.position = c(0.80,0.69),
             legend.title = element_blank(),legend.margin=margin(-0.2,0.1,0,0, "cm"),
             strip.text = element_text(margin = margin (3,0,3,0)),panel.spacing = unit(0, "lines"),
             strip.background =element_rect(fill=rgb(159,176,193,maxColorValue = 255))))
Plot_table <- Plot_info[!is.na(Plot_info$DNAStim),] %>% group_by(DisttoDNA,DNAStim)  %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  ungroup() %>% complete(DisttoDNA,DNAStim, fill = list(n=0,freq=0))
p6 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoDNA,y=n,fill=DNAStim)) +
       geom_bar(stat='identity') +
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(0,28),breaks=c(0,5,10,15,20,25)) +
       scale_fill_manual(values=c("#377EB8","#F8766D", "#4DAF4A")) +
       theme_bw() +
       labs(x = 'Distance to Default Network A (mm)',y = '# of Sites') + 
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             panel.grid.minor = element_blank(),
             text = element_text(size = 12),legend.position = c(0.80,0.69),
             legend.title = element_blank(),legend.margin=margin(-0.2,0.1,0,0, "cm"),
             strip.text = element_text(margin = margin (3,0,3,0)),panel.spacing = unit(0, "lines"),
             strip.background =element_rect(fill=rgb(159,176,193,maxColorValue = 255))))
g <- grid.arrange(p1,p2,p3,p4,p5,p6,ncol=1)
ggsave('/Users/cce3182/Desktop/CCEP_FM_Combined_Project/Figures/hehe.png',
       plot = g,width = 7.5,height = 10.5,units = 'in', dpi = 300)

Plot_table <- Plot_info[Plot_info$SMStim == 'Sensorimotor Network Activation',]
Plot_table$AmpResult <- factor(Plot_table$AmpResult, levels=c('Hit + Specific','Hit + Nonspecific','Miss + Specific','Miss + Nonspecific','No Effect'))
sensorimotormodel <- lm( DisttoWMBoundary~DisttoUNI, Plot_table)
summary(sensorimotormodel)
p2 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoUNI)) +
       annotate('rect',xmin=-Inf,xmax=Inf,ymin=0,ymax=Inf, fill='gray',alpha=0.5) +
       geom_hline(yintercept = 0, linewidth=0.25) +
       geom_line(aes(y=DisttoWMBoundary),stat="smooth",method = "lm", formula = y ~x, color='black', linewidth = 1,alpha = 0.5) + 
       geom_point(aes(y=DisttoWMBoundary,color=AmpResult),size=0.5, alpha=0.5) + facet_wrap(~SMStim) + 
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(-12,8),breaks=c(-10,-5,0,5)) +
         scale_color_manual(values=c('red','green','blue','yellow','black'), drop=FALSE) +
       theme_bw() + guides(fill='none', color='none') +
       labs(x = 'Distance to Sensorimotor Network (mm)', y = 'White Matter Displacement (mm)') + 
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             panel.grid.minor = element_blank(),axis.title.x = element_text(vjust = 1, hjust = 0.5),
...
Plot_table <- Plot_info[Plot_info$CGOPStim == 'Cingulo-Opercular Network Activation',]
Plot_table$AmpResult <- factor(Plot_table$AmpResult, levels=c('Hit + Specific','Hit + Nonspecific','Miss + Specific','Miss + Nonspecific','No Effect'))
CGOPmodel <- lm( DisttoWMBoundary~DisttoSAL, Plot_table)
summary(CGOPmodel)
p3 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoSAL)) +
       annotate('rect',xmin=-Inf,xmax=Inf,ymin=0,ymax=Inf, fill='gray',alpha=0.5) +
       geom_hline(yintercept = 0, linewidth=0.25) +
       geom_line(aes(y=DisttoWMBoundary),stat="smooth",method = "lm", formula = y ~x, color='black', linewidth = 1,alpha = 0.5) + 
       geom_point(aes(y=DisttoWMBoundary,color=AmpResult),size=0.5, alpha=0.5) + facet_wrap(~CGOPStim) + 
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(-12,8),breaks=c(-10,-5,0,5)) +
         scale_color_manual(values=c('red','green','blue','yellow','black'), drop=FALSE) +
       theme_bw() + guides(fill='none', color='none') +
       labs(x = 'Distance to Cingulo-Opercular Network (mm)', y = 'White Matter Displacement (mm)') + 
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             panel.grid.minor = element_blank(),axis.title.x = element_text(vjust = 1, hjust = 0.5),
...
Plot_table <- Plot_info[Plot_info$FPNAStim == 'Frontoparietal Network A Activation',]
Plot_table$AmpResult <- factor(Plot_table$AmpResult, levels=c('Hit + Specific','Hit + Nonspecific','Miss + Specific','Miss + Nonspecific','No Effect'))
FPNAmodel <- lm( DisttoWMBoundary~DisttoFPNA, Plot_table)
summary(FPNAmodel)
p4 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoFPNA)) +
       annotate('rect',xmin=-Inf,xmax=Inf,ymin=0,ymax=Inf, fill='gray',alpha=0.5) +
       geom_hline(yintercept = 0, linewidth=0.25) +
       geom_line(aes(y=DisttoWMBoundary),stat="smooth",method = "lm", formula = y ~x, color='black', linewidth = 1,alpha = 0.5) + 
       geom_point(aes(y=DisttoWMBoundary,color=AmpResult),size=0.5, alpha=0.5) + facet_wrap(~FPNAStim) + 
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(-12,8),breaks=c(-10,-5,0,5)) +
         scale_color_manual(values=c('red','green','blue','yellow','black'), drop=FALSE) +
       theme_bw() + guides(fill='none', color='none') +
       labs(x = 'Distance to Frontoparietal Network A (mm)', y = 'White Matter Displacement (mm)') + 
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             panel.grid.minor = element_blank(),axis.title.x = element_text(vjust = 1, hjust = 0.5),
             text = element_text(size = 12),axis.title.y = element_blank(),
             strip.text = element_text(margin = margin (3,0,3,0)),panel.spacing = unit(0, "lines"),
...
Plot_table <- Plot_info[Plot_info$LANGStim == 'Language Network Activation',]
Plot_table$AmpResult <- factor(Plot_table$AmpResult, levels=c('Hit + Specific','Hit + Nonspecific','Miss + Specific','Miss + Nonspecific','No Effect'))
LANGmodel <- lm( DisttoWMBoundary~DisttoLANG, Plot_table)
summary(LANGmodel)
p5 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoLANG)) +
       annotate('rect',xmin=-Inf,xmax=Inf,ymin=0,ymax=Inf, fill='gray',alpha=0.5) +
       geom_hline(yintercept = 0, linewidth=0.25) +
       geom_line(aes(y=DisttoWMBoundary),stat="smooth",method = "lm", formula = y ~x, color='black', linewidth = 1,alpha = 0.5) + 
       geom_point(aes(y=DisttoWMBoundary,color=AmpResult),size=0.5, alpha=0.5) + facet_wrap(~LANGStim) + 
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(-12,8),breaks=c(-10,-5,0,5)) +
         scale_color_manual(values=c('red','green','blue','yellow','black'), drop=FALSE) +
       theme_bw() + guides(fill='none', color='none') +
...
Plot_table <- Plot_info[Plot_info$DNBStim == 'Default Network B Activation',]
Plot_table$AmpResult <- factor(Plot_table$AmpResult, levels=c('Hit + Specific','Hit + Nonspecific','Miss + Specific','Miss + Nonspecific','No Effect'))
DNBmodel <- lm( DisttoWMBoundary~DisttoDNB, Plot_table)
summary(DNBmodel)
p6 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoDNB)) +
       annotate('rect',xmin=-Inf,xmax=Inf,ymin=0,ymax=Inf, fill='gray',alpha=0.5) +
       geom_hline(yintercept = 0, linewidth=0.25) +
       geom_line(aes(y=DisttoWMBoundary),stat="smooth",method = "lm", formula = y ~x, color='black', linewidth = 1,alpha = 0.5) + 
       geom_point(aes(y=DisttoWMBoundary,color=AmpResult),size=0.5, alpha=0.5) + facet_wrap(~DNBStim) + 
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(-12,8),breaks=c(-10,-5,0,5)) +
         scale_color_manual(values=c('red','green','blue','yellow','black'),  drop=FALSE) +
       theme_bw() + guides(fill='none', color='none') +
       labs(x = 'Distance to Default Network B (mm)', y = 'White Matter Displacement (mm)') + 
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             panel.grid.minor = element_blank(),axis.title.x = element_text(vjust = 1, hjust = 0.5),
             text = element_text(size = 12),axis.title.y = element_blank(),
             strip.text = element_text(margin = margin (3,0,3,0)),panel.spacing = unit(0, "lines"),
...
Plot_table <- Plot_info[Plot_info$DNAStim == 'Default Network A Activation',]
Plot_table$AmpResult <- factor(Plot_table$AmpResult, levels=c('Hit + Specific','Hit + Nonspecific','Miss + Specific','Miss + Nonspecific','No Effect'))
DNAmodel <- lm( DisttoWMBoundary~DisttoDNA, Plot_table)
summary(DNAmodel)
p7 <- ggplotGrob(ggplot(data = Plot_table, aes(x=DisttoDNA)) +
       annotate('rect',xmin=-Inf,xmax=Inf,ymin=0,ymax=Inf, fill='gray',alpha=0.5) +
       geom_hline(yintercept = 0, linewidth=0.25) +
       geom_line(aes(y=DisttoWMBoundary),stat="smooth",method = "lm", formula = y ~x, color='black', linewidth = 1,alpha = 0.5) + 
       geom_point(aes(y=DisttoWMBoundary,color=AmpResult),size=0.5, alpha=0.5) + facet_wrap(~DNAStim) + 
       scale_x_continuous(limits=c(0,22),breaks=c(0,5,10,15,20)) +
       scale_y_continuous(limits=c(-12,8),breaks=c(-10,-5,0,5)) +
       scale_color_manual(values=c('red','green','blue','yellow','black'), drop=FALSE) +
       theme_bw() + guides(fill='none') +
       labs(x = 'Distance to Default Network A (mm)', y = 'White Matter Displacement (mm)', color = '# of Stimulated Networks') + 
       theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(), 
             panel.grid.minor = element_blank(),axis.title.x = element_text(vjust = 1, hjust = 0.5),
             ...
             strip.text = element_text(margin = margin (3,0,3,0)),panel.spacing = unit(0, "lines"),
             strip.background =element_rect(fill=rgb(159,176,193,maxColorValue = 255))))
g <- grid.arrange(p2,p3,p4,p5,p6,p7,ncol=1, heights=c(1.65,1.65,1.65,1.65,1.65,2.25))
ggsave('/Users/cce3182/Desktop/CCEP_FM_Combined_Project/Figures/hehe1.png',
       plot = g,width = 7.5,height = 10.5,units = 'in', dpi = 300)


Plot_info <- CCEP_Data[!CCEP_Data$TissueType %in% c('Subcortical','Out of Brain') &
                         !CCEP_Data$AmpResult %in% c('Not Stimulated','No In-Network Coverage'),]
Plot_info$p <- Plot_info$t_test < 0.05
Plot_info <- Plot_info %>% group_by(Targeted_network, AmpResult,p) %>% count()
Plot_info$AmpResult <- factor(Plot_info$AmpResult,levels = c('No Effect','Hit + Specific',
                                                             'Hit + Nonspecific','Miss + Specific','Miss + Nonspecific'))
ggplot(Plot_info, aes(x=AmpResult,y=n,fill=p)) + 
             geom_bar(stat='identity') +
             scale_fill_manual(values=c('#00BFC4','#F8766D')) + #facet_wrap(~Targeted_network) +
             theme_bw() +
             labs(y = '# of Stimulation Sites',fill='Sig. Larger Magnitude') + 
             theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(),
                   panel.grid.minor = element_blank(),legend.margin=margin(0.1,0.1,0.1,0.1, "cm"),
                   legend.box.background = element_rect(colour = "black"),legend.position=c(0.766,0.75),
                   axis.text.x = element_text(angle=30,vjust=1,hjust=1),
...
Plot_info$AmpResult[Plot_info$AmpResult %in% c('Miss + Specific','Miss + Nonspecific')] <- 'Miss'
Plot_info$AmpResult <- factor(Plot_info$AmpResult, levels = c('Hit','Miss'))
ggplot(data = Plot_info, aes(x=as.numeric(AmpResult),y=NearNetworks, color = AmpResult)) +
         ...
         scale_fill_manual(values=c('#F8766D','#00BFC4')) +
         coord_cartesian(xlim=c(0.5,2.5)) + 
         theme_bw() + facet_wrap(~Targeted_network) + 
         labs(y = '# of Nearby Networks') + 
         theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(),
               panel.grid.minor = element_blank(),
               axis.ticks.x=element_blank(),axis.title.x=element_blank(),
...
Plot_info <- CCEP_Data[!CCEP_Data$TissueType %in% c('Subcortical','Out of Brain') &
                         !CCEP_Data$AmpResult %in% c('No In-Network Coverage'),]
Plot_info$AmpResult <- factor(Plot_info$AmpResult, levels = c('Hit + Specific','Hit + Nonspecific',
                                                              'Miss + Specific','Miss + Nonspecific',
                                                              'No Effect','Not Stimulated'))
ggplot(data = Plot_info, aes(x=NearNetworks,y=DisttoWMBoundary, color = AmpResult)) +
     annotate('rect',xmin=-Inf,xmax=Inf,ymin=0,ymax=Inf, fill='gray',alpha=0.5) +
     geom_hline(yintercept = 0, linewidth=0.25) +
     stat_density2d(geom = "polygon", aes(fill=after_stat(level), alpha=after_stat(level)), show.legend = FALSE) +
     geom_point(color='black', alpha=0.5, show.legend = FALSE) +
     facet_wrap(~AmpResult) +
     scale_y_continuous(limits=c(-13,5.5), breaks=seq(-10,5,by=5)) +
     theme_bw() +
     labs(y = 'White Matter Displacement (mm)', x = '# of Nearby Networks') + 
     theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(),
           panel.grid.minor = element_blank(),
           axis.ticks.x=element_blank(),
           text = element_text(size=12))
...
Plot_info <- CCEP_Data[!CCEP_Data$TissueType %in% c('Subcortical','Out of Brain') &
                         !CCEP_Data$AmpResult %in% c('No In-Network Coverage','Not Stimulated'),]
Plot_info$AmpResult <- factor(Plot_info$AmpResult, levels = c('Hit + Specific','Hit + Nonspecific',
                                                              'Miss + Specific','Miss + Nonspecific',
                                                              'No Effect'))
ggplot(data = Plot_info, aes(x=Targeted_network_percentage,y=DisttoWMBoundary, color = AmpResult)) +
  annotate('rect',xmin=-Inf,xmax=Inf,ymin=0,ymax=Inf, fill='gray',alpha=0.5) +
  geom_hline(yintercept = 0, linewidth=0.25) +
  #stat_density2d(geom = "polygon", aes(fill=after_stat(level), alpha=after_stat(level)), show.legend = FALSE) +
  #geom_point(color='black', alpha=0.5, show.legend = FALSE) +
  ...
  scale_y_continuous(limits=c(-13,5.5), breaks=seq(-10,5,by=5)) +
  theme_bw() +
  labs(y = 'White Matter Displacement (mm)', x = 'Targeted Network ROI %') + 
  theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(),
        panel.grid.minor = element_blank(),
        axis.ticks.x=element_blank(),
        text = element_text(size=12))
ggplot(data = Plot_info, aes(x=DisttoTargetedNetwork,y=DisttoWMBoundary, color = AmpResult)) +
  annotate('rect',xmin=-Inf,xmax=Inf,ymin=0,ymax=Inf, fill='gray',alpha=0.5) +
  geom_hline(yintercept = 0, linewidth=0.25) +
  stat_density2d(geom = "polygon", aes(fill=after_stat(level), alpha=after_stat(level)), show.legend = FALSE) +
  geom_point(color='black', alpha=0.5, show.legend = FALSE) +
  ...
  scale_y_continuous(limits=c(-13,5.5), breaks=seq(-10,5,by=5)) +
  theme_bw() +
  labs(y = 'White Matter Displacement (mm)', x = 'Distance to Targeted Network (mm)') + 
  theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_blank(),
        panel.grid.minor = element_blank(),
        axis.ticks.x=element_blank(),
        text = element_text(size=12))
