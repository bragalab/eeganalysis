CCEP_CaseList <- function(AverageInfo_Table){
  ...
  for (i in 1:dim(case_list)[1]){
    ...
    current_indices <- AverageInfo_Table$SubjectID==case_list$SubjectID[i] & 
      ...
      AverageInfo_Table$Identity != 'Unknown'
    ...
    if (sum(current_data$Category == 'Within-Network') >= 1){
      case_list$t_test[i] <- wilcox.test(current_data$Response_Magnitude[current_data$Category=='Within-Network'],
                                         current_data$Response_Magnitude[current_data$Category=='Across-Network'],
  ...
  sites_of_interest <- c(8,160,4,100,176)
  ...
  Plot_info$Response_Network <- factor(Plot_info$Response_Network, levels = c('DNA','DNB','LANG','FPNA','SAL','UNI'))
